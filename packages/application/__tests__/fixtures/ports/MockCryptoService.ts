...
const sha256 = async (data: Uint8Array): Promise<Uint8Array> => {
  const digest = await getCrypto().subtle.digest('SHA-256', data.buffer as ArrayBuffer);
...
export class MockCryptoService implements CryptoServicePort {
  ...
  async verify(data: Uint8Array, signature: Uint8Array, publicKey: Uint8Array): Promise<boolean> {
    ...
    return expected.length === signature.length && expected.every((value, index) => value === signature[index]);
  }
}
