...
export function validateRegisterUserCommand(command: RegisterUserCommand): CommandResult<ValidatedRegisterUserCommand> {
  ...
  const encryptionPublicKey = validateBytes(command.encryptionPublicKey, 'encryptionPublicKey', errors);
  const timestamp = validateTimestamp(command.timestamp, 'timestamp', errors);

  if (errors.length > 0 || !actorId || !signingPublicKey || !encryptionPublicKey || !timestamp) {
...
const validateBytes = (value: Uint8Array, field: string, errors: ValidationError[]): Uint8Array | null => {
