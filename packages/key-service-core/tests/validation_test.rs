...
use mo_key_service_core::formats::{
  decode_resource_grant_v1, decode_scope_state_v1, encode_resource_grant_v1, encode_scope_state_v1,
  KeyVaultHeaderV1, KeyVaultRecordContainerV1, KeyVaultRecordPlainV1, ResourceGrantV1, ScopeStateV1, VaultKeyWrapV1,
};
use mo_key_service_core::keyvault::{make_store_scope_key_record, KeyVaultState};
use mo_key_service_core::types::{AeadId, DeviceId, ResourceId, ResourceKeyId, ScopeId, SigCiphersuiteId};
...
fn make_containers() -> (KeyVaultHeaderV1, Vec<u8>, Vec<KeyVaultRecordContainerV1>) {
  ...
  let record1: KeyVaultRecordPlainV1 = make_store_scope_key_record("rec-1", "scope-1", 1, &[9u8; 32]);
  let c1 = state
    .append_record(&header, &vault_key, &record1, 1)
    .expect("append record 1");
  let record2: KeyVaultRecordPlainV1 = make_store_scope_key_record("rec-2", "scope-1", 2, &[8u8; 32]);
  let c2 = state
