...
fn apply_record_plain(record: &KeyVaultRecordPlainV1, materialized: &mut KeyVaultMaterialized) -> CoreResult<()> {
  match record.kind {
    ...
    3 => {
      ...
      materialized.scope_keys.insert((scope_id.0, scope_epoch.0), scope_key);
    }
    4 => {
      let map = crate::cbor::as_map(&record.payload)?;
...
pub fn make_store_user_key_record(record_id: &str, uk_priv: &[u8], uk_pub: &[u8]) -> KeyVaultRecordPlainV1 {
...
pub fn resource_key_lookup_key(resource_id: &ResourceId, resource_key_id: &ResourceKeyId) -> (String, String) {
