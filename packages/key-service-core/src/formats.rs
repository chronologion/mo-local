use crate::cbor::{
  as_array, as_map, cbor_array, cbor_bytes, cbor_map, cbor_text, cbor_uint, decode_canonical_value,
...
pub fn encode_keyvault_header_v1(header: &KeyVaultHeaderV1) -> Result<Vec<u8>, String> {
  ...
  let value = cbor_map(vec![
    ...
    (5, cbor_array(header.records.iter().map(|r| encode_record_container_value(r)).collect())),
    (6, vault_key_wrap),
  ]);
  encode_canonical_value(&value)
}
