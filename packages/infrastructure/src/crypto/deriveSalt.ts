...
export const encodeSalt = (salt: Uint8Array): string => btoa(String.fromCharCode(...Array.from(salt)));

export const decodeSalt = (saltB64: string): Uint8Array => {
  const decoded = Uint8Array.from(atob(saltB64), (c) => c.charCodeAt(0));
  if (decoded.length < 16) {
    throw new Error('Invalid salt: minimum 16 bytes required per NIST SP 800-132');
  }
