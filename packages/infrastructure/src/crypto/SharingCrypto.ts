...
export class SharingCrypto {
  ...
  async deriveSharedSecret(privateKey: Uint8Array, peerPublicKey: Uint8Array): Promise<Uint8Array> {
    ...
    const privateKeyHandle = await subtle.importKey('pkcs8', privateBytes, { name: 'ECDH', namedCurve: CURVE }, false, [
      'deriveBits',
    ]);
    const publicKeyHandle = await subtle.importKey('raw', publicBytes, { name: 'ECDH', namedCurve: CURVE }, false, []);

    const bits = await subtle.deriveBits({ name: 'ECDH', public: publicKeyHandle }, privateKeyHandle, 256);
    return new Uint8Array(bits);
  }

  async wrapForRecipient(params: WrapParams): Promise<Uint8Array> {
    const sharedSecret = await this.deriveSharedSecret(params.senderPrivateKey, params.recipientPublicKey);
  ...
  async unwrapFromSender(params: UnwrapParams): Promise<Uint8Array> {
    const sharedSecret = await this.deriveSharedSecret(params.recipientPrivateKey, params.senderPublicKey);
    const wrappingKey = await this.crypto.deriveKey(sharedSecret, 'kgoal-wrap');
