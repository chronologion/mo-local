...
export class DomainToEncryptedEventAdapter {
  ...
  async toEncrypted(
    ...
    try {
      encryptedPayload = await this.crypto.encrypt(payloadBytes, aggregateKey, aad);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown encryption error';
      throw new Error(`Failed to encrypt ${domainEvent.eventType} for ${domainEvent.aggregateId.value}: ${message}`);
    }
  ...
  async toEncryptedBatch(
    ...
    return Promise.all(domainEvents.map((event, idx) => this.toEncrypted(event, startVersion + idx, aggregateKey)));
  }
}
