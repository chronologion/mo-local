import { BadRequestException, Body, Controller, Get, Headers, Logger, Post, Res, UseGuards } from '@nestjs/common';
...
import { SESSION_COOKIE_MAX_AGE_MS, SESSION_COOKIE_NAME, SESSION_COOKIE_SECURE, parseCookies } from '../session-cookie';

const getRequestHost = (res: Response): string | null => {
  const req = res.req;
  if (!req) return null;
  const hostHeader = req.headers?.host;
  const host = typeof hostHeader === 'string' ? hostHeader : Array.isArray(hostHeader) ? hostHeader[0] : null;
  if (!host) return null;
...
export class AuthController {
  ...
  whoami(@AuthIdentity() identity: AuthenticatedIdentity | undefined) {
    if (!identity) {
      throw new BadRequestException('Authenticated identity missing from context');
    }
    const email = typeof identity.traits.email === 'string' ? identity.traits.email : undefined;
  ...
  async logout(
    ...
    const token =
      ...
      parseCookies(res.req.headers.cookie as string | undefined)[SESSION_COOKIE_NAME];
