...
For now, prefer (1) for “Documents/Memories” unless we discover hard rotation requirements.
...
- [ ] Where should key distribution records live: embedded in event stream (like `keyring_update`) vs a dedicated Access stream/collection?
...
- [ ] Do we need per-item keys (wrapped) for any BC, or can we start with HKDF-derived keys everywhere?
